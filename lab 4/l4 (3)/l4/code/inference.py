...
def constructBayesNet(gameState: hunters.GameState):
    ...
    "*** YOUR CODE HERE ***"
    variables.extend((PAC,GHOST0,GHOST1,OBS0,OBS1))#adding the varialbes
 

    edges.append((GHOST0,OBS0))#adding edge as show in the problems
    ...
    #appending all the possible values for pacman and ghost into a dictoniary
    ...
    n = MAX_NOISE + X_RANGE + Y_RANGE -1

    for x in range(n):
        variableDomainsDict[OBS1].append(x)
        variableDomainsDict[OBS0].append(x)
...
def inferenceByVariableEliminationWithCallTracking(callTrackingList=None):
    ...
    def inferenceByVariableElimination(bayesNet: bn, queryVariables: List[str], evidenceDict: Dict, eliminationOrder: List[str]):
        ...
        "*** YOUR CODE HERE ***"
        currentFactorsList = bayesNet.getAllCPTsWithEvidence(evidenceDict)
        l = len(eliminationOrder)
        i=0
        while(i<l):
             variable=eliminationOrder[i]
             ...
             if len(joinedFactor.unconditionedVariables()) > 1:
                marginInc = eliminate(joinedFactor, variable)
                currentFactorsList.append(marginInc)
             i=i+1
        allJoint = joinFactors(currentFactorsList)
        cond = normalize(allJoint)
